...
class Services:
    ...
    def allocate_investment_funds(
        ...
        if free_amount_object > free_amount_model:
            ...
            Services.close_object_for_donation(model)
        elif free_amount_object == free_amount_model:
            Services.close_object_for_donation(obj_in)
            Services.close_object_for_donation(model)
        else:
    ...
    async def investments_process(
        ...
        objects_model = await Services.get_uninvested_objects(model, session)

        for model in objects_model:
            obj_in, model = Services.allocate_investment_funds(obj_in, model)
            session.add(obj_in)
            session.add(model)
    ...
    async def create_object(
        ...
        await Services.investments_process(new_object, model, session)
        return new_object
